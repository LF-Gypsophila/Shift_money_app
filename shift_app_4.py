##背景テーマのプリセット＆背景データの記憶 ・絞り込みフィルタ & 並び替え ・メモ欄の追加
...
def save_settings(limit_income, fiscal_start, theme_name=None):
    """設定＋背景画像も JSON に保存"""
    settings = {
...
if "shifts" not in st.session_state or len(st.session_state["shifts"]) == 0:
    ...
    save_settings(limit_income, fiscal_start, theme_name)
    raise SystemExit
...
# ページ2:シフト一覧(表＋扶養チェック＋削除&複製)
    ...
    with col_f1:
        filter_start = st.date_input(
            "開始日フィルタ", value=min_date, min_value=min_date, max_value=max_date
        )
    with col_f2:
        filter_end = st.date_input(
            "終了日フィルタ", value=max_date, min_value=min_date, max_value=max_date
        )
    with col_f3:
