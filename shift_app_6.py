## 1レコードの給与計算専用関数:calc_pay_for_shiftを追加
## ComentOut
...
def load_settings() -> Optional[Dict[str, Any]]:
    ...
    global WORKPLACE_SETTINGS #グローバルな勤務先設定を更新するためglobal宣言
...
if page == "カレンダー": #カレンダーページ
   ...
   #カレンダー表示用テーブルデータ（HTML＋ツールチップ）
    ...
    def highlight_calendar(val: str) -> str:
        """カレンダーセルの色付け（給与を含むセルの背景を変える）"""
        if isinstance(val, str) and "円" in val: #"円" を含むセルはシフトあり
    ...
    st.markdown('<div class="calendar-card">', unsafe_allow_html=True) #カード開始
    cal_styler = cal_df.style.map(highlight_calendar) #セルに色付けするStyler
    st.markdown(cal_styler.to_html(), unsafe_allow_html=True) #HTMLとしてレンダリング
