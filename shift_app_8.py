...
WORKPLACE_SETTINGS = { #各バイト先ごとのルールやデフォルト時給などを定義
    "すたば": { #バイト先「すたば」の設定
        ...
        "default_transport": 640, #デフォルト交通費
...
def get_default_transport_for_workplace(workplace: str) -> int:
    """
    勤務先ごとのデフォルト交通費を返すヘルパー。
    WORKPLACE_SETTINGS に default_transport が設定されていなければ 0 を返す。
    """
    settings = WORKPLACE_SETTINGS.get(workplace, {})
    return int(settings.get("default_transport", 0))
       ...
       #パターンに応じてデフォルト値を決める
        ...
        with col2:
           #時給のデフォルト値：パターン優先、なければ履歴から自動計算
            ...
            #交通費のデフォルト設定:パターン優先 なければ勤務先設定から
            if pattern is not None:
                default_transport = int(pattern.get("transport", 
                                                    get_default_transport_for_workplace(workplace)))
            else:
                default_transport = get_default_transport_for_workplace(workplace)
    ...
    for wp_name in sorted(WORKPLACE_SETTINGS.keys()): #勤務先ごとに設定フォームを表示
        ...
        with st.expander(f"{wp_name} の設定", expanded=False): #折りたたみの枠
            ...
            with col1:
                ...
                default_transport = st.number_input(
                    f"{wp_name} の基本交通費（往復）",
                    min_value=0,
                    value=int(settings_wp.get("default_transport", 0)),
                    step=10,
                    key=f"{wp_name}_default_transport",
                ) #基本交通費入力
           ...
           #入力値を即マスタに反映
            ...
            settings_wp["default_transport"] = int(default_transport)
